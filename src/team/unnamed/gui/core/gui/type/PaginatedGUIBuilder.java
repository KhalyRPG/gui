...
public class PaginatedGUIBuilder<E> extends GUIBuilderLayout<PaginatedGUIBuilder<E>> {
	...
	private final Set<Function<Integer, ItemClickable>> itemsReplacingWithPage = new HashSet<>();
	...
	public PaginatedGUIBuilder<E> setItemsPerRow(int amount) {
		isState(amount > 0 && amount <= 9, "Items per row cannot be equals than 0 or major than 9.");
	...
	public Inventory build() {
		...
		PaginatedGUIData<E> guiData = new PaginatedGUIData<>(slots, new ArrayList<>(Arrays.asList(items)), openAction,
				closeAction, cancelClick, originalTitle, entities, itemParser, 1, from, to, skippedSlotsInBounds,
				itemsPerRow, previousPageItem, nextPageItem, itemIfNotEntities, itemIfNotPreviousPage,
